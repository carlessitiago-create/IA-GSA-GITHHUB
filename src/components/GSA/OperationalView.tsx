...
export const OperationalView: React.FC = () => {
  ...
  const isAdm =
    profile?.nivel?.startsWith("ADM") || profile?.nivel === "ADM_ANALISTA";
  ...
  const [selectedPendenciaProcess, setSelectedPendenciaProcess] =
    useState<OrderProcess | null>(null);
  ...
  useEffect(() => {
    const loadClientAndSync = async () => {
      if (selectedProcess) {
        ...
        if (
          client &&
          (!selectedProcess.cliente_cpf_cnpj ||
            !selectedProcess.data_nascimento ||
            !selectedProcess.cliente_nome)
        ) {
          console.log(
            "Detectados campos de segurança ausentes no processo, sincronizando...",
          );
          try {
            ...
            if (Object.keys(updates).length > 0) {
              await updateDoc(
                fsDoc(db, "order_processes", selectedProcess.id!),
                updates,
              );
              // Atualiza o estado local para refletir a mudança
              setSelectedProcess((prev) =>
                prev?.id === selectedProcess.id
                  ? { ...prev, ...updates }
                  : prev,
              );
            }
  ...
  const handleDownloadPDF = async (processo: OrderProcess) => {
    ...
    try {
      ...
      if (!cliente) {
        console.warn(
          "Dados do cliente não encontrados na base, usando dados do processo como fallback.",
        );
      ...
      const modelo = processo.modelo_id
        ? await obterModeloProcesso(processo.modelo_id)
        : null;
  ...
  const handleDeleteProcess = async (processo: OrderProcess) => {
    if (!isAdm) {
      Swal.fire(
        "Acesso Restrito",
        "Apenas administradores podem excluir processos.",
        "error",
      );
      return;
    }
  ...
  const selecionarResponsavel = async (
    role: string,
    field: "vendedor" | "gestor" | "analista",
  ) => {
    ...
    try {
      ...
      if (role === "ADM_ANALISTA") {
        const snap2 = await getDocs(
          query(collection(db, "usuarios"), where("nivel", "==", "ADM_MASTER")),
        );
      ...
      if (userId) {
        ...
        setProcessos((prev) =>
          prev.map((p) =>
            p.id === selectedProcess.id ? { ...p, ...updates } : p,
          ),
        );
  ...
  const handleMudarServico = async () => {
    ...
    try {
      ...
      if (
        selectedServicoId &&
        selectedServicoId !== selectedProcess.servico_id
      ) {
        const servicoSelecionado: any = servicosAtivos.find(
          (s) => s.id === selectedServicoId,
        );
        ...
        await registrarLogAuditoria(
          selectedProcess.id!,
          `Tipo de processo alterado para: ${servicoSelecionado.nome}`,
          profile?.uid || "",
          profile?.nome_completo || "Analista",
        );
    ...
    if (novoStatus === "Concluído") {
        ...
        preConfirm: async (file) => {
          if (!file) {
            Swal.showValidationMessage(
              "Você precisa selecionar um arquivo para anexar!",
            );
            return false;
          }
          try {
      ...
      if (isConfirmed || isDenied) {
        ...
        try {
          ...
          Swal.fire(
            "Erro",
            error.message || "Falha ao concluir processo.",
            "error",
          );
        }
      }
  ...
  const handleAtribuirCliente = async (processo: OrderProcess) => {
    if (!isAdm) {
      Swal.fire(
        "Acesso Restrito",
        "Apenas administradores ou analistas podem atribuir processos manualmente.",
        "error",
      );
      return;
    }
    ...
    const { value: inputCpf } = await Swal.fire({
      ...
      input: "text",
      inputValue: processo.cliente_cpf_cnpj || "",
    ...
    if (inputCpf) {
      try {
        ...
        const { getDocs, query, collection, where, doc, updateDoc } =
          await import("firebase/firestore");
        ...
        if (snap.empty) {
          Swal.fire(
            "Não Encontrado",
            "Nenhum usuário localizado com este documento.",
            "warning",
          );
          return;
        }
        ...
        if (isConfirmed && processo.id) {
          const procsUpdates: any = {
            cliente_id: userId,
            cliente_cpf_cnpj: cleanDoc,
            cliente_nome: userDoc.nome_completo,
          };
          if (
            processo.visibilidade_uids &&
            !processo.visibilidade_uids.includes(userId)
          ) {
            procsUpdates.visibilidade_uids = [
              ...processo.visibilidade_uids,
              userId,
            ];
          } else if (!processo.visibilidade_uids) {
            procsUpdates.visibilidade_uids = [userId];
          }

          await updateDoc(
            doc(db, "order_processes", processo.id),
            procsUpdates,
          );
  ...
  const handleAbrirPendencia = async (processo: OrderProcess) => {
    if (!isAdm) {
      Swal.fire(
        "Acesso Restrito",
        "Apenas analistas podem reprovar documentos ou abrir pendências.",
        "error",
      );
      return;
    }
  ...
  const submitNovaPendencia = async (descricao: string, tipo?: string) => {
    ...
    try {
      ...
      const { notificarPendenciaManual } =
        await import("../../services/notificationService");
      await notificarPendenciaManual(
        selectedPendenciaProcess,
        selectedPendenciaProcess.cliente_id,
        descricao,
      );

      Swal.fire(
        "Pendência Aberta",
        "O cliente e o vendedor responsável já receberam o alerta.",
        "warning",
      );
  ...
  const handleNotificarPendencias = async (processo: OrderProcess) => {
    ...
    if (totalCampos === 0 && faltamDocs <= 0) {
      Swal.fire(
        "Tudo em Ordem",
        "Não há pendências de ficha técnica para este processo.",
        "info",
      );
      return;
    }
    ...
    try {
      const { notificarPendenciaFicha } =
        await import("../../services/notificationService");
      await notificarPendenciaFicha(
        processo,
        processo.cliente_id,
        totalCampos,
        Math.max(0, faltamDocs),
      );
      ...
      Swal.fire(
        "Erro",
        error.message || "Falha ao enviar notificação.",
        "error",
      );
    }
  };
  ...
  const handleConfirmarRecebimentoManual = async (
    processo: OrderProcess,
    docKey: string,
  ) => {
    const docLabel =
      requirementsConfig?.document_labels?.[docKey] ||
      docKey.replace(/_/g, " ");
    const result = await Swal.fire({
        ...
        if (!currentDocs.includes(docKey)) {
          ...
          if (isProcessReady(updatedProcess)) {
            if (
              ...
              updatedProcess.status_atual === "Aguardando Documentação"
            ) {
              ...
              Swal.fire(
                "Pronto!",
                "Todos os documentos e dados foram preenchidos. Processo agora está Em Análise.",
                "success",
              );
            } else {
              await updateDoc(doc(db, "order_processes", processo.id!), {
            ...
            if (
              ...
              Swal.fire(
                "Atenção",
                "A documentação ainda está incompleta. O status retornou para Pendente.",
                "warning",
              );
            } else {
              await updateDoc(doc(db, "order_processes", processo.id!), {
              ...
              Swal.fire(
                "Sucesso!",
                "Documento confirmado. Faltam outros requisitos.",
                "info",
              );
            }
          }
          ...
          setProcessos((prev) =>
            prev.map((p) => (p.id === processo.id ? updatedProcess : p)),
          );
  ...
  const isProcessReady = (processo: OrderProcess) => {
    ...
    const hasTrackingData =
      !!processo.cliente_cpf_cnpj && !!processo.data_nascimento;

    // Apenas OK se enviou TODOS os documentos exigidos e preencheu TODOS os campos requeridos
    const docsReady = reqDocs.every((d) =>
      (processo.documentos_enviados || []).includes(d),
    );
  ...
  const filteredProcessos = processos.filter((p) => {
    ...
    const matchesStatus =
      filterStatus === "all" || p.status_atual === filterStatus;

    return matchesSearch && matchesStatus;
  });

  const totalFila = processos.filter(
    (p) => p.status_atual !== "Concluído",
  ).length;
  const totalPendente = processos.filter(
    (p) => p.status_atual === "Pendente",
  ).length;
  const totalEmAnalise = processos.filter(
    (p) => p.status_atual === "Em Análise",
  ).length;
  const totalProtocolado = processos.filter(
    (p) => p.status_atual === "Protocolado",
  ).length;
  const totalEmAndamento = processos.filter(
    (p) => p.status_atual === "Em Andamento",
  ).length;
  const totalAguardandoDoc = processos.filter(
    (p) => p.status_atual === "Aguardando Documentação",
  ).length;
  ...
  const totalAtraso = processos.filter((p) => {
    ...
    const dataInicio = p.data_inicial?.toDate
      ? p.data_inicial.toDate()
      : new Date();
  ...
  const totalConcluidoHoje = processos.filter((p) => {
    if (p.status_atual !== "Concluído") return false;
    const dataConclusao = p.data_conclusao_real?.toDate
      ? p.data_conclusao_real.toDate()
      : new Date();
    ...
    <div className="space-y-10 pb-20">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-8 bg-gradient-to-br from-[#020617] to-[#0a0a2e] p-6 md:p-6 rounded-2xl md:rounded-3xl border border-slate-800 shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative overflow-hidden group">
          ...
          <div className="flex items-center gap-3 bg-[#0F172A] w-fit px-3 py-1.5 rounded-lg border border-slate-800">
            ...
            <p className="text-slate-400 text-xs font-semibold tracking-wide">
              ...
              <span className="text-white">
                {auth.currentUser?.displayName || "Analista GSA"}
              </span>
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-8 gap-2 sm:gap-3 relative z-10 w-full mt-4 lg:mt-0">
          <div className="bg-[#0B0F19] px-3 md:px-4 py-3 rounded-2xl border border-slate-800/50 shadow-inner flex flex-col items-center justify-center relative overflow-hidden group/card shadow-[0_0_20px_rgba(0,0,0,0.5)]">
            ...
            <p className="text-[9px] sm:text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1 text-center whitespace-nowrap">
              Em Fila
            </p>
            <p className="text-xl md:text-2xl font-bold text-slate-400 tracking-tight drop-shadow-[0_0_10px_rgba(148,163,184,0.3)]">
              {displayFila.toString().padStart(2, "0")}
            </p>
          </div>
          <div className="bg-[#0B0F19] px-3 md:px-4 py-3 rounded-2xl border border-blue-900/20 shadow-inner flex flex-col items-center justify-center relative overflow-hidden group/card shadow-[0_0_20px_rgba(0,0,0,0.5)]">
            ...
            <p className="text-[9px] sm:text-[10px] font-bold text-blue-500/70 uppercase tracking-widest mb-1 text-center whitespace-nowrap">
              Pendente
            </p>
            <p className="text-xl md:text-2xl font-bold text-blue-500 tracking-tight drop-shadow-[0_0_10px_rgba(59,130,246,0.3)]">
              {displayPendente.toString().padStart(2, "0")}
            </p>
          </div>
          <div className="bg-[#0B0F19] px-3 md:px-4 py-3 rounded-2xl border border-cyan-900/20 shadow-inner flex flex-col items-center justify-center relative overflow-hidden group/card shadow-[0_0_20px_rgba(0,0,0,0.5)]">
            ...
            <p className="text-[9px] sm:text-[10px] font-bold text-cyan-500/70 uppercase tracking-widest mb-1 text-center whitespace-nowrap">
              Em Análise
            </p>
            <p className="text-xl md:text-2xl font-bold text-cyan-500 tracking-tight drop-shadow-[0_0_10px_rgba(6,182,212,0.3)]">
              {displayEmAnalise.toString().padStart(2, "0")}
            </p>
          </div>
          <div className="bg-[#0B0F19] px-3 md:px-4 py-3 rounded-2xl border border-amber-900/20 shadow-inner flex flex-col items-center justify-center relative overflow-hidden group/card shadow-[0_0_20px_rgba(0,0,0,0.5)]">
            ...
            <p className="text-[9px] sm:text-[10px] font-bold text-amber-500/70 uppercase tracking-widest mb-1 text-center whitespace-nowrap">
              Aguar. Doc
            </p>
            <p className="text-xl md:text-2xl font-bold text-amber-500 tracking-tight drop-shadow-[0_0_10px_rgba(245,158,11,0.3)]">
              {displayAguardandoDoc.toString().padStart(2, "0")}
            </p>
          </div>
          <div className="bg-[#0B0F19] px-3 md:px-4 py-3 rounded-2xl border border-orange-900/20 shadow-inner flex flex-col items-center justify-center relative overflow-hidden group/card shadow-[0_0_20px_rgba(0,0,0,0.5)]">
            ...
            <p className="text-[9px] sm:text-[10px] font-bold text-orange-500/70 uppercase tracking-widest mb-1 text-center whitespace-nowrap">
              Andamento
            </p>
            <p className="text-xl md:text-2xl font-bold text-orange-500 tracking-tight drop-shadow-[0_0_10px_rgba(249,115,22,0.3)]">
              {displayEmAndamento.toString().padStart(2, "0")}
            </p>
          </div>
          <div className="bg-[#0B0F19] px-3 md:px-4 py-3 rounded-2xl border border-indigo-900/20 shadow-inner flex flex-col items-center justify-center relative overflow-hidden group/card shadow-[0_0_20px_rgba(0,0,0,0.5)]">
            ...
            <p className="text-[9px] sm:text-[10px] font-bold text-indigo-500/70 uppercase tracking-widest mb-1 text-center whitespace-nowrap">
              Protocolo
            </p>
            <p className="text-xl md:text-2xl font-bold text-indigo-500 tracking-tight drop-shadow-[0_0_10px_rgba(99,102,241,0.3)]">
              {displayProtocolado.toString().padStart(2, "0")}
            </p>
          </div>
          <div className="bg-[#0B0F19] px-3 md:px-4 py-3 rounded-2xl border border-emerald-900/20 shadow-inner flex flex-col items-center justify-center relative overflow-hidden group/card shadow-[0_0_20px_rgba(0,0,0,0.5)]">
            ...
            <p className="text-[9px] sm:text-[10px] font-bold text-emerald-500/70 uppercase tracking-widest mb-1 text-center whitespace-nowrap">
              Concluído
            </p>
            <p className="text-xl md:text-2xl font-bold text-emerald-500 tracking-tight drop-shadow-[0_0_10px_rgba(52,211,153,0.3)]">
              {displayConcluido.toString().padStart(2, "0")}
            </p>
          </div>
          <div className="bg-[#0B0F19] px-3 md:px-4 py-3 rounded-2xl border border-rose-900/20 shadow-inner flex flex-col items-center justify-center relative overflow-hidden group/card shadow-[0_0_20px_rgba(0,0,0,0.5)]">
            ...
            <p className="text-[9px] sm:text-[10px] font-bold text-rose-500/70 uppercase tracking-widest mb-1 text-center whitespace-nowrap">
              Atraso SLA
            </p>
            <p className="text-xl md:text-2xl font-bold text-rose-500 tracking-tight drop-shadow-[0_0_10px_rgba(225,29,72,0.3)]">
              {displayAtraso.toString().padStart(2, "0")}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 items-center">
        ...
        >
          ...
          <option value="Aguardando Documentação">
      ...
      <div className="grid grid-cols-1 gap-6">
        {filteredProcessos.map((processo, idx) => {
          const dataVenda = processo.data_venda?.toDate
            ? processo.data_venda.toDate()
            : new Date();
          const diasDesdeVenda = Math.ceil(
            Math.abs(new Date().getTime() - dataVenda.getTime()) /
              (1000 * 60 * 60 * 24),
          );
          ...
          if (processo.data_inicial) {
            ...
            const slaDiffTime = Math.abs(
              new Date().getTime() - dataInicio.getTime(),
            );
          ...
          const ready =
            isProcessReady(processo) &&
            !["Pendente", "Aguardando Documentação"].includes(
          ...
          return (
            ...
            >
              ...
              <div className="space-y-2 min-w-[200px] w-full lg:w-auto">
                <div className="flex flex-wrap items-center gap-2">
                  <div className="bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100">
                    <span className="text-[10px] font-bold text-blue-700 uppercase tracking-wide">
                      #
                      {processo.protocolo ||
                        processo.id?.slice(-6).toUpperCase()}
                    </span>
                  </div>
                  {isAtrasado && (
                  ...
                  {processo.cliente_id &&
                  processo.cliente_id !== "STANDALONE" &&
                  processo.cliente_id.length > 20 ? (
                    <div
                      className="bg-indigo-100 px-2 py-0.5 rounded-md border border-indigo-200"
                      title="O cliente tem acesso ao portal"
                    >
                      <span className="text-[10px] font-bold text-indigo-700 uppercase tracking-wide flex items-center gap-1">
                        <UserCheck size={10} /> Atribuído
                      </span>
                    </div>
                  ) : (
                    <button
                      onClick={() => handleAtribuirCliente(processo)}
                      className="bg-slate-100 px-2 py-0.5 rounded-md border border-slate-200 hover:bg-slate-200 transition-colors"
                      title="Clique para atribuir a um cliente existente ou buscar automaticamente pelo CPF"
                    >
                      <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide flex items-center gap-1 cursor-pointer">
                        <User size={10} /> S/ Conta
                      </span>
                    </button>
                  )}
                </div>
                <div className="flex flex-col flex-1 min-w-[200px] w-full lg:w-auto">
                  {(processo.cliente_cpf_cnpj &&
                    processo.cliente_cpf_cnpj.length > 11) ||
                  (processo as any).nome_empresa ? (
                    <>
                      ...
                      <div className="flex flex-col gap-1 mt-1">
                        <p className="text-[11px] font-medium text-slate-500">
                          ...
                          <span className="font-mono text-slate-700">
                            {processo.cliente_cpf_cnpj?.replace(
                              /^(\d{2})(\d{3})(\d{3})(\d{4})(\d{2}).*/,
                              "$1.$2.$3/$4-$5",
                            )}
                          </span>
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 text-[10px] font-medium text-slate-500 mt-2">
                  ...
                  <p>
                    Gestor:{" "}
                    <span className="text-slate-800">
                      {processo.gestor_nome || "N/A"}
                    </span>
                  </p>
                  <p>
                    Ana:{" "}
                    <span className="text-slate-800">
                      {processo.analista_nome || "N/A"}
                    </span>
                  </p>
                  <p>
                    Há:{" "}
                    <span className="text-blue-600 font-semibold">
                      {processo.data_status_atual
                        ? `${Math.ceil((new Date().getTime() - processo.data_status_atual.toDate().getTime()) / (1000 * 60 * 60 * 24))} dias`
                        : "0 dias"}
                    </span>
                  </p>
                </div>
                ...
                {!ready && (
                  <div className="flex flex-wrap gap-2 mt-2">
                    {!processo.cliente_cpf_cnpj && (
                      <span className="text-[10px] font-semibold text-rose-700 bg-rose-50 px-2 py-0.5 rounded border border-rose-200 truncate">
                        Falta CPF
                      </span>
                    )}
                    ...
                    {processo.dados_faltantes?.map((f) => (
                      <span
                        key={f}
                        className="text-[10px] font-semibold text-amber-700 bg-amber-50 px-2 py-0.5 rounded border border-amber-200 truncate max-w-[150px]"
                      >
                        Falta:{" "}
                        {requirementsConfig?.field_labels?.[f] ||
                          f.replace(/_/g, " ")}
                      </span>
                    ))}
                    {processo.pendencias_iniciais
                      ?.filter(
                        (d) => !processo.documentos_enviados?.includes(d),
                      )
                      .map((d) => (
                        <span
                          key={d}
                        ...
                        >
                          Pendente:{" "}
                          {requirementsConfig?.document_labels?.[d] ||
                            d.replace(/_/g, " ")}
                        </span>
                      ))}
                  </div>
                ...
                {isAdm ? (
                  <div className="relative group/select">
                    <select
                      ...
                      onChange={(e) =>
                        handleUpdateStatus(processo, e.target.value as any)
                      }
                ...
                <div className="text-right space-y-1">
                  ...
                  <div className="flex items-center justify-end gap-1.5 text-slate-700">
                    ...
                    <p className="text-[11px] font-medium">
                      {processo.data_venda?.toDate
                        ? format(processo.data_venda.toDate(), "dd/MM/yy")
                        : "N/A"}
                    </p>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 lg:flex gap-2 w-full lg:w-auto mt-4 lg:mt-0">
                ...
                >
                  <FolderOpen size={16} />
                  <span className="ml-2 lg:hidden text-xs font-semibold">
                    Pasta
                  </span>
                </button>
                {isAdm && (
        ...
        {filteredProcessos.length === 0 && (
          <div className="py-32 text-center bg-white rounded-3xl border-2 border-dashed border-slate-100 shadow-inner">
            ...
            <h3 className="text-2xl font-black text-slate-300 uppercase tracking-tighter italic">
              Nenhum processo na fila de produção
            </h3>
            <p className="text-slate-400 text-sm mt-2 font-medium">
              Aguarde novas vendas concluídas para iniciar a operação.
            </p>
          </div>
        )}
      </div>
              ...
              <div className="p-6 md:p-6 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
                <div className="flex items-center gap-4 md:gap-6">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-lg md:text-3xl font-black text-[#0a0a2e] uppercase tracking-tighter italic truncate">
                      Auditoria
                    </h3>
                    <p className="text-[8px] md:text-[10px] font-black text-slate-400 uppercase tracking-widest truncate">
                      Doc: #
                      {selectedProcess.protocolo ||
                        selectedProcess.id?.slice(-6).toUpperCase()}
                    </p>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 md:p-6 space-y-6 md:space-y-10 custom-scrollbar">
                ...
                <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-8">
                      ...
                      {isAdm && (
                        <button
                          onClick={async () => {
                            ...
                            if (
                              novoNome &&
                              novoNome !== selectedProcess.cliente_nome
                            ) {
                              const { updateCliente } =
                              ...
                              await updateCliente(selectedProcess.cliente_id, {
                                nome: novoNome,
                              });
                              await updateDoc(
                                fsDoc(
                              ...
                              await registrarLogAuditoria(
                                selectedProcess.id!,
                                `Nome do cliente alterado para: ${novoNome}`,
                                profile?.uid || "",
                                profile?.nome_completo || "Analista",
                              );
                              setSelectedProcess({
                    ...
                    <div className="flex flex-wrap gap-4 md:gap-6">
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2">
                          <p className="text-xs md:text-sm font-black text-slate-600 uppercase tracking-tight">
                            {selectedProcess.cliente_cpf_cnpj || "N/A"}
                          </p>
                          {isAdm && (
                            <button
                              onClick={async () => {
                                ...
                                if (novoDoc !== undefined) {
                                  ...
                                  for (const c of cpfsToSearch) {
                                    ...
                                    if (!snapUser.empty) {
                                      finalClienteId = snapUser.docs[0].id;
                                      break;
                                    }
                                  }
                                  ...
                                  if (
                                    selectedProcess.cliente_id &&
                                    selectedProcess.cliente_id !== "STANDALONE"
                                  ) {
                                    await updateCliente(
                                      selectedProcess.cliente_id,
                                      { documento: cleanDoc },
                                  ...
                                  if (
                                    ...
                                    finalClienteId !==
                                      selectedProcess.cliente_id &&
                                    finalClienteId !== "STANDALONE"
                                  ) {
                                    processUpdates.cliente_id = finalClienteId;
                                    if (
                                      selectedProcess.visibilidade_uids &&
                                      !selectedProcess.visibilidade_uids.includes(
                                        finalClienteId,
                                      )
                                    ) {
                                      processUpdates.visibilidade_uids = [
                                        ...selectedProcess.visibilidade_uids,
                                        finalClienteId,
                                      ];
                                    } else if (
                                  ...
                                  if (
                                    ...
                                    try {
                                      ...
                                      console.warn("Could not update sales", e);
                                    }
                                  }

                                  await registrarLogAuditoria(
                                    selectedProcess.id!,
                                    `CPF/CNPJ do cliente alterado para: ${cleanDoc}${finalClienteId !== selectedProcess.cliente_id ? " (Processo vinculado automaticamente à conta do cliente)" : ""}`,
                                    profile?.uid || "",
                                    profile?.nome_completo || "Analista",
                                  );
                                  ...
                                  Swal.fire(
                                    ...
                                    "Documento alterado com sucesso." +
                                      (finalClienteId !==
                                        selectedProcess.cliente_id &&
                                      finalClienteId !== "STANDALONE"
                                        ? " O processo foi vinculado automaticamente à conta do cliente."
                                        : ""),
                                    "success",
                                  );
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-2">
                          <p className="text-xs md:text-sm font-black text-slate-600 uppercase tracking-tight">
                            {selectedProcess.data_nascimento || "N/A"}
                          </p>
                          {isAdm && (
                            <button
                              onClick={async () => {
                                ...
                                if (novaData !== undefined) {
                                  ...
                                  await registrarLogAuditoria(
                                    selectedProcess.id!,
                                    `Data de nascimento alterada para: ${novaData}`,
                                    profile?.uid || "",
                                    profile?.nome_completo || "Analista",
                                  );
                                  setSelectedProcess({
                      ...
                      {isAdm && (
                        <div className="flex gap-4 md:gap-6 flex-wrap">
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="flex items-center gap-2">
                              <p className="text-xs md:text-sm font-black text-slate-600 uppercase tracking-tight truncate max-w-[150px]">
                  ...
                  <div className="md:col-span-4 bg-[#0a0a2e] p-6 md:p-8 rounded-3xl md:rounded-2xl text-white flex flex-col justify-center shadow-2xl shadow-blue-900/30">
                    <span className="text-[9px] font-black text-blue-300 uppercase tracking-widest mb-1 md:mb-2">
                      Status
                    </span>
                    <h4 className="text-xl md:text-2xl font-black uppercase italic leading-none text-blue-400">
                ...
                <div className="bg-emerald-50 p-6 md:p-8 rounded-3xl md:rounded-2xl border border-emerald-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  <div>
                    <span className="text-[9px] font-black text-emerald-500 uppercase tracking-widest mb-1 md:mb-2 block">
                  ...
                  {isAdm && (
                    <button
                      onClick={handleMudarServico}
                      className="px-5 py-2.5 bg-white text-emerald-600 border border-emerald-200 hover:bg-emerald-600 hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-sm shrink-0 whitespace-nowrap"
                    >
                      Alterar Serviço
                    </button>
                  )}
                ...
                <div className="bg-slate-50 p-4 md:p-6 rounded-2xl border border-slate-100 grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-black text-slate-700 truncate">
                        {selectedProcess.vendedor_nome || "N/A"}
                      </p>
                      {isAdm && (
                        <button
                          onClick={() =>
                            selecionarResponsavel("VENDEDOR", "vendedor")
                          }
                          className="p-1 hover:bg-slate-200 rounded-md text-slate-400 hover:text-blue-600 transition-colors shrink-0"
                        >
                          <Edit3 size={12} />
                        </button>
                      )}
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-black text-slate-700 truncate">
                        {selectedProcess.gestor_nome || "N/A"}
                      </p>
                      {isAdm && (
                        <button
                          onClick={() =>
                            selecionarResponsavel("GESTOR", "gestor")
                          }
                          className="p-1 hover:bg-slate-200 rounded-md text-slate-400 hover:text-blue-600 transition-colors shrink-0"
                        >
                          <Edit3 size={12} />
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      <p className="text-sm font-black text-slate-700 truncate">
                        {selectedProcess.analista_nome || "N/A"}
                      </p>
                      {isAdm && (
                        <button
                          onClick={() =>
                            selecionarResponsavel("ADM_ANALISTA", "analista")
                          }
                          className="p-1 hover:bg-slate-200 rounded-md text-slate-400 hover:text-blue-600 transition-colors shrink-0"
                        >
                          <Edit3 size={12} />
                  ...
                  <div className="w-full lg:w-[35%] flex flex-col space-y-4 md:space-y-6">
                    <div className="flex items-center justify-between px-2">
                      <div className="flex items-center gap-3">
                        ...
                        <h4 className="text-sm md:text-lg font-black text-[#0a0a2e] uppercase tracking-tighter italic whitespace-nowrap">
                          Documentos
                        </h4>
                      </div>
                      <div className="bg-blue-50 px-3 md:px-4 py-1.5 rounded-full border border-blue-100 shrink-0">
                        <span className="text-[8px] md:text-[10px] font-black text-blue-600 uppercase tracking-widest">
                          {selectedProcess.documentos_enviados?.length || 0}/
                          {selectedProcess.pendencias_iniciais?.length || 0} OK
                        </span>
                      </div>
                    </div>
                    <div className="flex-1 grid grid-cols-1 gap-3 md:gap-4 overflow-y-visible lg:overflow-y-auto pr-0 lg:pr-4 custom-scrollbar content-start">
                      {selectedProcess.pendencias_iniciais?.map((docKey) => {
                        const isEnviado =
                          selectedProcess.documentos_enviados?.includes(docKey);
                        return (
                          <div
                            key={docKey}
                          ...
                          >
                            <div className="flex items-center gap-3 md:gap-4 min-w-0">
                              <div
                                className={`size-10 md:size-12 rounded-xl md:rounded-2xl flex items-center justify-center shadow-sm shrink-0 ${
                                  isEnviado
                              ...
                              <span
                                className={`text-[10px] md:text-[11px] font-black uppercase tracking-tight truncate ${
                                  isEnviado
                                    ? "text-emerald-700"
                                    : "text-slate-400"
                                }`}
                              >
                                {requirementsConfig?.document_labels?.[
                                  docKey
                                ] || docKey}
                              </span>
                            </div>
                            {isEnviado ? (
                              <div className="flex gap-2">
                                ...
                                {isAdm && (
                                  <button
                                    onClick={() =>
                                      handleAbrirPendencia(selectedProcess)
                                    }
                              ...
                              <button
                                onClick={() =>
                                  handleConfirmarRecebimentoManual(
                                    selectedProcess,
                                    docKey,
                                  )
                                }

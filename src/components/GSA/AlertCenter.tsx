      ...
      {allAvisos.map(aviso => (
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 sm:gap-8 relative z-10 text-center sm:text-left">
            <div className={`size-14 sm:size-20 rounded-2xl sm:rounded-[1.8rem] flex items-center justify-center text-white shadow-2xl animate-pulse shrink-0 ${
              aviso.type === 'process' ? 'bg-amber-500 shadow-amber-500/30' : 'bg-rose-500 shadow-rose-500/30'
            }`}>
                {aviso.type === 'process' ? <ClipboardList size={28} className="sm:size-[36px]" /> : <AlertCircle size={28} className="sm:size-[36px]" />}
            </div>
            <div className="space-y-1 min-w-0">
              <div className="flex items-center justify-center sm:justify-start gap-2 sm:gap-3">
                <h4 className={`font-black uppercase text-[8px] sm:text-[10px] tracking-[0.2em] sm:tracking-[0.3em] ${
                  aviso.type === 'process' ? 'text-amber-600' : 'text-rose-600'
                }`}>
                  {aviso.type === 'process' ? 'Pendência Operacional' : 'Alerta GSA IA'}
                </h4>
                <div className={`size-2 rounded-full animate-ping ${
                  aviso.type === 'process' ? 'bg-amber-500' : 'bg-rose-500'
                }`} />
              </div>
              <p className="text-[#0a0a2e] font-black text-xl sm:text-2xl md:text-3xl italic tracking-tighter leading-none truncate max-w-full">
                {aviso.type === 'process' ? aviso.cliente_nome : `Aguardando: ${aviso.clienteNome}`}
              </p>
              <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest mt-1 sm:mt-2 line-clamp-2">

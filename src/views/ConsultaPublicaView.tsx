...
export function ConsultaPublicaView() {
              ...
              if (p.id === docSnap.id) {
                ...
                if (
                  (statusAnterior === "Pendente" ||
                    statusAnterior === "Aguardando Pagamento") &&
                  statusNovo !== "Pendente" &&
                  statusNovo !== "Aguardando Pagamento"
                ) {
                  confetti({
                    particleCount: 150,
                    spread: 100,
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setSearched(true);
    try {
      const { consultaPublicaProcesso } =
        await import("../services/publicService");
      const process = await consultaPublicaProcesso(
        searchTerm,
        dataNascimento,
        codigoInterno,
      );
      ...
      if (process) {
        ...
        const { listarMinhasIndicacoesPublicas } =
          await import("../services/publicService");
        const refs = await listarMinhasIndicacoesPublicas(
          searchTerm,
          dataNascimento,
          codigoInterno,
        );
      ...
      Swal.fire(
        "Erro",
        "Falha na conexão com o banco de dados. Tente novamente.",
        "error",
      );
    } finally {
      setLoading(false);
    ...
    <div className="p-4 sm:p-8 max-w-7xl mx-auto space-y-12 pb-32">
        ...
        <div className="bg-blue-50 border border-blue-100 p-6 rounded-3xl flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-blue-700 mt-1">
              Este painel é para uso interno e consulta rápida. Seus clientes
              devem acessar o portal público oficial pelo link:
            </p>
            <div className="mt-3 flex items-center gap-3">
              ...
              <button
                onClick={() => {
                  ...
                  Swal.fire(
                    "Copiado!",
                    "Link encaminhado para a área de transferência.",
                    "success",
                  );
                }}
      ...
      <div className="max-w-2xl mx-auto space-y-8">
        <div className="text-center space-y-3">
          ...
          <h1 className="text-2xl font-black text-[#0a0a2e] uppercase italic tracking-tighter leading-none">
            Acompanhar Processo
          </h1>
          <p className="text-slate-400 text-xs font-bold uppercase tracking-widest leading-relaxed">
            Digite os dados para acesso imediato ao status do cliente.
          </p>
        </div>

        <form
          onSubmit={handleSearch}
          className="bg-white p-2 rounded-2xl shadow-2xl border border-slate-100"
        >
          <div className="bg-slate-50 rounded-[calc(2.5rem-8px)] p-6 sm:p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest flex items-center gap-2">
                <User size={12} className="text-blue-600" /> Documento ou
                Protocolo
              </label>
              <input
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest flex items-center gap-2">
                  <Calendar size={12} className="text-blue-600" /> Data Nasc.
                  (Opcional)
                </label>
                <input
                  type="date"
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest flex items-center gap-2">
                  <ShieldAlert size={12} className="text-blue-600" /> Código
                  Interno (Opcional)
                </label>
                <input
                  type="text"
      ...
      <AnimatePresence mode="wait">
        {searched && !loading && results.length === 0 && (
          ...
          >
            ...
            <h3 className="text-2xl font-black text-[#0a0a2e] uppercase italic tracking-tighter">
            ...
            <p className="text-slate-500 text-sm font-bold uppercase tracking-widest max-w-sm mx-auto leading-relaxed">
              Verifique os dados digitados ou entre em contato com nosso suporte
              via WhatsApp.
            </p>
          </motion.div>
        )}
          ...
          >
            {results.map((processo) => (
              <div key={processo.id} className="space-y-16">
                ...
                <div className="bg-[#0a0a2e] rounded-3xl p-8 sm:p-6 text-white shadow-2xl relative overflow-hidden border border-white/10 group">
                  ...
                  <div className="relative z-10 flex flex-col md:flex-row items-center gap-6">
                    ...
                    <div className="text-center md:text-left space-y-2 flex-1">
                      <div className="flex items-center justify-center md:justify-start gap-2 mb-2">
                        <span className="text-[10px] font-black bg-blue-600 px-3 py-1 rounded-full uppercase tracking-widest">
                          Protocolo: {processo.protocolo || processo.id}
                        </span>
                      </div>
                      <h2 className="text-2xl sm:text-6xl font-black uppercase italic tracking-tighter leading-none">
                        {processo.cliente_nome}
                      </h2>
                      <p className="text-blue-400 text-lg font-black uppercase italic tracking-widest">
                        {processo.servico_nome}
                      </p>
                    </div>
                    <div className="flex flex-col items-end gap-2">
                      <span className="text-[10px] font-black text-blue-300 uppercase opacity-50 tracking-[0.2em]">
                        Progresso Atual
                      </span>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-12 relative z-10">
                    <div className="bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10 text-center">
                      ...
                      <p className="text-lg font-black text-white">
                        {processo.data_venda
                          ? format(processo.data_venda.toDate(), "dd/MM/yyyy", {
                              locale: ptBR,
                            })
                          : "-"}
                      </p>
                    ...
                    <div className="bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10 text-center">
                      ...
                      <p className="text-lg font-black text-white">
                        {processo.prazo_estimado_dias || 7} Dias Úteis
                      </p>
                    </div>
                    <div className="bg-white/5 backdrop-blur-md rounded-2xl p-6 border border-white/10 text-center">
                      <div className="size-10 bg-white/10 rounded-xl flex items-center justify-center mx-auto mb-3">
                      ...
                      <p className="text-lg font-black text-white uppercase italic">
                        {processo.vendedor_nome || "Equipe GSA"}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="mt-8 flex justify-center md:justify-end gap-4 overflow-hidden relative z-10">
                    {processo.cliente_uid && (
                      <button
                        onClick={() =>
                          handleSimulateClient(processo.cliente_uid)
                        }
                ...
                {(processo.status_atual === "Pendente" ||
                  processo.status_atual === "Aguardando Pagamento") && (
                  <div className="bg-amber-50 p-8 sm:p-6 rounded-3xl border-2 border-amber-200 shadow-2xl flex flex-col md:flex-row items-center gap-8 text-center md:text-left animate-pulse-slow">
                    ...
                    <div className="flex-1">
                      <h3 className="text-3xl font-black text-amber-900 uppercase italic leading-none">
                        Aguardando Compensação...
                      </h3>
                      <p className="text-amber-800 text-sm font-bold mt-3 leading-relaxed">
                        Seu pagamento foi pré-identificado ou está em
                        processamento pelos nossos gateways.
                        <strong>
                          {" "}
                          Esta tela atualizará sozinha assim que o banco
                          confirmar.
                        </strong>
                      </p>
                    </div>
                    ...
                    <div className="relative z-10 space-y-12">
                      <div className="text-center space-y-4">
                        ...
                        <p className="text-blue-200 text-sm sm:text-lg max-w-md mx-auto opacity-80 mt-4 leading-relaxed font-semibold uppercase italic">
                          Indique um amigo e ganhe bônus de R$ 50,00 por
                          contrato fechado diretamente na sua conta.
                        </p>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center bg-white/5 p-8 rounded-2xl border border-white/10 backdrop-blur-md">
                        <div className="space-y-4">
                          <button
                            onClick={() => {
                              navigator.clipboard.writeText(
                                `${getIndicaOrigin()}?ref=${searchTerm}`,
                              );
                              Swal.fire(
                                "Link Copiado!",
                          ...
                          <div className="pt-6 border-t border-white/10">
                            <p className="text-[10px] font-black uppercase text-blue-300 mb-4 tracking-widest">
                              Indicar Diretamente:
                            </p>
                            <div className="space-y-3">
                              ...
                              <button
                                onClick={() => {
                                  ...
                                  Swal.fire(
                                    "Indicação Enviada!",
                                    "Nossa equipe entrará em contato com seu amigo.",
                                    "success",
                                  );
                                  (
                                    document.getElementById(
                        ...
                        <div className="bg-white/10 p-8 rounded-3xl space-y-6 text-center md:text-left h-full flex flex-col justify-center">
                          <div className="flex justify-between items-center border-b border-white/10 pb-4">
                            ...
                            <span className="text-3xl font-black text-emerald-400 italic">
                  ...
                  <div className="bg-amber-50 p-6 rounded-3xl border-4 border-amber-200 shadow-2xl space-y-8 animate-pulse-slow">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-amber-800 text-xs font-bold uppercase tracking-widest mt-2">
                          Resolva as pendências abaixo para não atrasar seu
                          processo.
                        </p>
                      </div>
                    </div>
                ...
                <div className="space-y-10">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h2 className="text-3xl font-black text-[#0a0a2e] uppercase italic tracking-tighter">
                        Elite de Prêmios
                      </h2>
                      <p className="text-slate-400 text-xs font-bold uppercase tracking-widest">
                        Transforme pontos em presentes reais e bônus na conta.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    {[
                      {
                        nome: "VALE COMPRAS - BOTICÁRIO R$25",
                        pts: 500,
                        img: "https://images.unsplash.com/photo-1596462502278-27bfdc4033c8?auto=format&fit=crop&q=80&w=400",
                      },
                      {
                        nome: "VALE COMPRAS - SHOPEE R$25",
                        pts: 500,
                        img: "https://images.unsplash.com/photo-1622547748225-3fc4abd2cca0?auto=format&fit=crop&q=80&w=400",
                      },
                      {
                        nome: "VALE CRÉDITO - UBER R$25",
                        pts: 500,
                        img: "https://images.unsplash.com/photo-1510605395823-530474d7490e?auto=format&fit=crop&q=80&w=400",
                      },
                      {
                        nome: "VALE COMPRAS - IFOOD R$25",
                        pts: 500,
                        img: "https://images.unsplash.com/photo-1513104890138-7c749659a591?auto=format&fit=crop&q=80&w=400",
                      },
                      {
                        nome: "R$ 50 OFF - SERVIÇOS GSA",
                        pts: 1000,
                      ...
                      },
                      {
                        nome: "PIX R$100",
                        pts: 2000,
                        img: "https://images.unsplash.com/photo-1580519542036-c47de6196ba5?auto=format&fit=crop&q=80&w=400",
                      },
                      {
                        nome: "PIX R$200",
                        pts: 4000,
                      ...
                      >
                        ...
                        <h4 className="text-[10px] font-black text-slate-800 uppercase italic truncate">
                          {p.nome}
                        </h4>
                  ...
                  <button
                    onClick={() =>
                      Swal.fire(
                        "Portal de Prêmios",
                        "O catálogo completo será liberado para o cliente em sua área VIP exclusiva.",
                        "info",
                      )
                    }
                ...
                {/* Clube de Prêmios Promo */}
                <div className="mt-8">
                  <ClubePromoBanner
                    isPublic
                    onAction={() => window.open(getConsultaOrigin(), "_blank")}
                  />
                </div>
                ...
                <div className="bg-white rounded-3xl p-6 border border-slate-100 shadow-2xl relative overflow-hidden text-center space-y-8">
                  ...
                  <div className="relative z-10 max-w-2xl mx-auto space-y-6">
                    ...
                    <p className="text-slate-500 text-sm font-medium italic">
                      Fique sabendo de novas oportunidades, bônus e notícias do
                    ...
                    >
                      ...
                      <div className="sm:col-span-2 flex flex-col sm:flex-row gap-4">
                        ...
                        >
                          {newsLoading
                            ? "Inscrevendo..."
                            : "Cadastrar na Lista VIP GSA"}
                        </button>
                        <button
                          type="button"
                          onClick={() =>
                            window.open(
                              "https://chat.whatsapp.com/ExempleGroupGSA",
                              "_blank",
                            )
                          }

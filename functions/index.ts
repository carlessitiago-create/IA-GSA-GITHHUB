...
async function processarBonusDeVendaNoBackend(
  vendaId: string,
...
function cleanDataForFirestore(obj: any, isRoot = true): any {
  ...
  if (Array.isArray(obj))
    return obj
      .map((v) => cleanDataForFirestore(v, false))
...
function throwHttpsError(
  ...
  const detailMessage = technicalDetails
    ? `${message} [Info: ${technicalDetails}]`
    : message;
  console.error(
    `[HTTPS_ERROR] Code: ${safeCode} | Msg: ${message} | Technical: ${technicalDetails}`,
  );
...
function safeExecute(
  ...
  return async (request: CallableRequest) => {
    try {
      ...
      console.error(
        `[${moduleName}] Emitindo Erro -> Code: ${safeCode}, Message: ${safeMessage}`,
      );
...
export const criarAdministradorDeUsuarios = onCall(async (request) => {
  ...
  try {
    const userRecord = await admin
      .auth()
      .createUser({ email, password: senha || "Mudar@123", displayName: nome });
    const newProfile = {
      uid: userRecord.uid,
      nome,
      email,
    ...
    await db
      .collection("usuarios")
      .doc(userRecord.uid)
      .set(cleanDataForFirestore(newProfile));
    return { uid: userRecord.uid };
  } catch (error: any) {
    throw new HttpsError(
      "aborted",
      "Erro ao criar usuário: " + (error?.message || error),
    );
  }
});
...
export const atualizarSenhaUsuario = onCall(async (request) => {
  ...
  const callerSnap = await db
    ...
    .get();
  if (
    !["ADM_MASTER", "ADM_MESTRE", "ADM_GERENTE"].includes(
  ...
  try {
    ...
    throw new HttpsError(
      "aborted",
      "Erro ao atualizar senha: " + (error?.message || error),
    );
  }
});
...
export const syncUsers = onCall(async (request) => {
  ...
  try {
    ...
    throw new HttpsError(
      "aborted",
      "Erro ao sincronizar usuários: " + (error?.message || error),
    );
  }
});
  ...
  safeExecute("PROCESS_VENDA", async (request) => {
    ...
    return await db.runTransaction(async (transaction: any) => {
      ...
      for (const item of itens) {
        if (!item.servicoId)
          throw new HttpsError("invalid-argument", "servicoId ausente");

        const servicoSnap = await transaction.get(
          db.doc(`services/${item.servicoId}`),
        );
        let servicoData: any = servicoSnap.exists ? servicoSnap.data() : null;

        if (!servicoData) {
          if (
            vendedorId === "SYSTEM_SAAS" ||
            ["diag_credito", "diag_saas"].includes(item.servicoId)
          ) {
            servicoData = {
        ...
        let pendenciasIniciais =
          servicoData?.requisitos_documentos || servicoData?.documentos || [];
        let dadosFaltantes =
          servicoData?.requisitos_campos || servicoData?.campos || [];

        if (modeloId) {
          const modelSnap = await transaction.get(
            db.doc(`process_models/${modeloId}`),
          );
          if (modelSnap.exists) {
        ...
        processDataList.push({
          ...
          servicoNome: item.servicoNome || servicoData.nome,
          precoBase,
          precoVenda,
      ...
      const clientSnap = await transaction.get(
        db.collection("clients").doc(clienteId),
      );
      ...
      let statusPagamento =
        metodoPagamento === "PIX" ? "Aguardando Confirmação" : "Pendente";
      let statusProcesso =
        metodoPagamento === "PIX" ? "Aguardando Aprovação" : "Pendente";
      ...
      if (metodoPagamento === "CARTEIRA" && walletId) {
        ...
        let saldoDisponivel =
          (walletData?.saldo_atual || 0) + (walletData?.saldo_bonus || 0);
      ...
      if (metodoPagamento === "PIX") {
        transaction.set(
          db.collection("financial_transactions").doc(),
          cleanDataForFirestore({
            cliente_id: clienteId,
            valor: valorTotal,
            tipo: "CREDITO",
        ...
        transaction.set(
          ...
          cleanDataForFirestore({
            ...
            tipo: "DEBITO",
            origem: "VENDA",
            descricao: transacaoConfirmada
              ? `Pagamento via Carteira - Venda ${saleRef.id.slice(0, 8)}`
              : `Aguardando Liberação - Venda ${saleRef.id.slice(0, 8)}`,
            confirmado_pelo_administrador: transacaoConfirmada,
            venda_id: saleRef.id,
            vendedor_id: vendedorId,
            vendedor_nome: vendedorNome,
            managerId,
            timestamp: FieldValue.serverTimestamp(),
            visibilidade_uids: transacaoConfirmada
              ? visibilidade_uids
...
export const processarVendaSegura = onCall(
  safeExecute("VENDA_SEGURA", async (request) => {
    ...
    return await db.runTransaction(async (tx: any) => {
      ...
      if (servicoId !== "manual") {
        const servicoSnap = await tx.get(
          db.collection("services").doc(servicoId),
        );
      ...
      if (valor < minimoTotal - 0.01)
        throw new HttpsError(
          "permission-denied",
          `Abaixo do mínimo (R$ ${valor} < R$ ${minimoTotal})`,
        );
      ...
      if (metodoPagamento === "CARTEIRA") {
        ...
        if (walletQuery.empty)
          throw new HttpsError("failed-precondition", "Cliente sem carteira");

        const walletRef = walletQuery.docs[0].ref;
        const wallet = (await tx.get(walletRef)).data()!;
        const totalDisponivel =
          (Number(wallet.saldo_atual) || 0) + (Number(wallet.saldo_bonus) || 0);

        if (totalDisponivel < valor)
          throw new HttpsError("failed-precondition", "Saldo insuficiente");

        let aDebitar = valor,
          novoBonus = Number(wallet.saldo_bonus) || 0,
          novoSaldo = Number(wallet.saldo_atual) || 0;
      ...
      if (user.id_superior) {
        const gestorSnap = await tx.get(
          db.collection("usuarios").doc(user.id_superior),
        );
      ...
      if (split_comissoes) {
        ...
        if (gestorUser) {
          ...
          // Valor baseado na Tabela de Custas (preço base do gestor para este serviço)
          let precoBaseTabela = Number(servico.preco_base_gestor) || 0;
          if (isBulk && servico.is_mass_sale_active) {
            precoBaseTabela =
              Number(servico.preco_massa_gestor) || precoBaseTabela;
          }

          const custo_fixo = precoBaseTabela * qty;
          const pct_lucro =
            (Number(gestorUser.comissao_especial_percentual) || 0) / 100;
      ...
      const visibilidade_uids = [
        uid,
        clienteId,
        ...(user.id_superior ? [user.id_superior] : []),
      ];
      ...
      tx.set(
        ...
        cleanDataForFirestore({
          ...
          valor_total: valor,
          margem_total: valor - (Number(servico.preco_base_gestor) || 0) * qty,
          metodo_pagamento: metodoPagamento,
          status_pagamento:
            metodoPagamento === "CARTEIRA" ? "Pago" : "Pendente",
          quantidade: qty,
      ...
      const statusProcesso =
        metodoPagamento === "CARTEIRA" ? "Em Análise" : "Aguardando Pagamento";
      for (let i = 0; i < qty; i++) {
        tx.set(
          db.collection("order_processes").doc(),
          cleanDataForFirestore({
            venda_id: saleRef.id,
            protocolo,
            servico_id: servicoId,
            ...
            pendencias_iniciais:
              servico.requisitos_documentos || servico.documentos || [],
...
export const gerarPagamentoPixGateway = onCall(
  safeExecute("PIX_GATEWAY", async (request) => {
    try {
      ...
      if (request.data?.origem !== "SAAS_LANDING_PAGE") throw e;
    }
    ...
    try {
      const response = await mpPayment.create({
        body: {
          ...
          payer: {
            ...
            identification: {
              type: String(cpf).replace(/\D/g, "").length > 11 ? "CNPJ" : "CPF",
      ...
      throw new HttpsError(
        "aborted",
        `Gateway recusou: ${err.response?.data?.message || err.message}`,
      );
    }
  }),
);
...
export const gerarPagamentoAsaas = onCall(
  safeExecute("ASAAS_PIX", async (request) => {
    ...
    if (!token)
      throw new HttpsError(
        "failed-precondition",
        "Configuração de pagamento Asaas não encontrada.",
      );

    const ASAAS_URL = config.is_sandbox
    ...
    console.log(
      `[ASAAS_PIX] Iniciando para venda: ${vendaId}, Cliente: ${email}`,
    );
    ...
    try {
      ...
      const searchRes = await axios.get(
        `${ASAAS_URL}/customers?email=${email}`,
        { headers },
      );
      ...
      const paymentData = {
        ...
        description:
          descricao || `Pagamento GSA Venda ${vendaId.substring(0, 8)}`,
      };
      ...
      return {
        ...
        qr_code_base64:
          qrRes.data.encodedImage || qrRes.data.qrCodeBase64 || "",
      ...
      console.error(
        "[ASAAS_PIX] Erro na API:",
        JSON.stringify(error.response?.data || error.message),
      );
      throw new HttpsError(
        "aborted",
        `Falha na integração Asaas: ${error.response?.data?.errors?.[0]?.description || error.message}`,
      );
    }
  }),
);
...
export const webhookMercadoPago = onRequest(
  ...
  async (req: any, res: any) => {
    ...
    try {
      ...
      const vendaId =
        paymentInfo.external_reference || paymentInfo.metadata?.venda_id;
...
export const registrarVendaAdministrativa = onCall(async (request) => {
  ...
  if (
    !["ADM_MASTER", "ADM_GERENTE", "ADM_ANALISTA"].includes(
      userSnap.data()?.nivel,
    )
  )
    throw new HttpsError("permission-denied", "Permissão negada.");
...
export const analyzeSmartFicha = onCall(async (request) => {
  ...
  `;
    ...
    const res = await axios.post(`https://generativelanguage.googleapis.com/v1/models/gemini-2.0-flash:generateContent?key=${apiKey}`, {
      generationConfig: {
...
export const analyzeDocument = onCall(async (request) => {
  ...
  .onUpdate(async (change, context) => {
    ...
    if (
      dadosDepois.is_limpa_nome &&
      dadosAntes.progresso_baixa !== 100 &&
      dadosDepois.progresso_baixa === 100
    ) {
...
export { webhookPagamentos } from "./src/webhookPagamentos";
